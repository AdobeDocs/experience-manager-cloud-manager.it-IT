---
title: Configurazioni del Dispatcher
...
# Configurazioni del Dispatcher {#manage-your-dispatcher-configurations}
...
Progetti generati da Cloud Manager utilizzando [creazione guidata progetto](/help/getting-started/using-the-wizard.md) la corretta struttura del progetto Maven viene creata automaticamente. Questo è il percorso consigliato per i nuovi utenti di Adobe Managed Services (AMS).

Una volta implementato in un’istanza del dispatcher, il contenuto di queste directory sull’istanza del Dispatcher viene sovrascritto da quelli nell’archivio Git. Poiché i file di configurazione del server web e del Dispatcher richiedono spesso informazioni specifiche per l’ambiente per poter utilizzare correttamente questa funzionalità, devi prima lavorare con i tecnici del successo cliente (CSE) per impostare queste variabili di ambiente in `/etc/sysconfig/httpd`.

## Configurazione del Dispatcher per i clienti esistenti di Managed Service {#steps-for-configuring-dispatcher}

Segui questi passaggi per completare la configurazione iniziale di Dispatcher.

1. Ottieni i file di configurazione di produzione correnti dal tuo CSE.
1. Rimuovi i dati specifici dell’ambiente codificati, ad esempio l’IP del renderer di pubblicazione, e sostituisci con le variabili.
1. Definisci le variabili richieste in coppie chiave-valore per ogni target Dispatcher e richiedi al tuo CSE di aggiungerle a `/etc/sysconfig/httpd` su ogni istanza.
1. Verifica le configurazioni aggiornate nell&#39;ambiente di staging.
1. Una volta testato, richiedi al tuo CSE di distribuire in produzione.
1. Invia i file al tuo archivio Git.
1. Distribuisci tramite Cloud Manager.

>[!NOTE]
>
>La migrazione delle configurazioni del Dispatcher e del server web all’archivio Git può essere eseguita durante l’onboarding di Cloud Manager, ma può anche essere eseguita in un secondo momento.

### Esempio {#example}

La struttura specifica del file e della directory può variare in base alle specifiche del progetto, ma questo esempio dovrebbe fornire una guida concreta alla struttura del progetto per includere le configurazioni di Apache e Dispatcher.

1. Creare una sottodirectory denominata `dispatcher`.

   È possibile utilizzare un nome qualsiasi, ma il nome della directory creato in questo passaggio deve essere lo stesso del nome utilizzato al punto 6.

1. Questa sottodirectory conterrà un modulo Maven che crea il file .zip del Dispatcher utilizzando il plugin Maven Assembly. Per iniziare, nella `dispatcher` directory, creare un `pom.xml` file con questo contenuto, modifica `parent` riferimento, `artifactId`e `name` se necessario.
   ...
   * Come nel Passaggio 1, l&#39;artifactId e il nome qui possono essere altri valori se lo desideri. `dispatcher` viene utilizzato qui solo come esempio.

1. Il plug-in Maven Assembly richiede un `descriptor` per definire la modalità di creazione del file .zip. Per creare questo descrittore, crea un file nel `dispatcher` sottodirectory denominata `assembly.xml` con il contenuto seguente. Tieni presente che a questo nome di file viene fatto riferimento alla riga 26 nel `pom.xml` file precedente.
...
1. Creare una sottodirectory denominata `src` (come indicato nel descrittore dell&#39;assembly alla riga 11) all&#39;interno della sottodirectory del dispatcher per memorizzare le configurazioni effettive di Apache e Dispatcher. All&#39;interno di `src` directory, creare directory denominate `conf`, `conf.d`, `conf.dispatcher.d`e `conf.modules.d`.

1. Popolare `conf`, `conf.d`, `conf.dispatcher.d`e `conf.modules.d` con i file di configurazione. Ad esempio, la configurazione predefinita è costituita da questi file e collegamenti simbolici.
...
1. Infine, nella `pom.xml` aggiungi un file nella directory principale del progetto `<module>` per includere il modulo dispatcher.

   Ad esempio, se l’elenco dei moduli esistenti è
   ...
   * Come indicato al punto 1, il valore `<module>` deve corrispondere al nome della directory creata.

1. Per eseguire il test, eseguire `mvn clean package` nella directory principale del progetto. Dovresti vedere linee come questa nell&#39;output.
